  ...
  server.registerTool(
    'create_time_entry',
    {
      ...
      inputSchema: z.object({
        ...
        service_id: z.number().int().positive().optional().describe('Service ID'),
        service_name: z
          .string()
          .optional()
          .describe('Service name (partial, case-insensitive)'),
        note: z.string().optional().describe('Entry note / description'),
      }),
    ...
    async (args) => {
      ...
      return {
        content: [
          {
            type: 'text',
            text: `Created time entry ${entry.id} (${fmtMinutes(entry.minutes)} on ${entry.date_at})`,
          },
        ],
...
function registerUpdateTool(server, session) {
  server.registerTool(
    'update_time_entry',
    {
      ...
      inputSchema: z.object({
        id: z.number().int().positive().describe('ID of the time entry to update'),
        minutes: z
          .number()
          .int()
          .positive()
        ...
        project_id: z.number().int().positive().optional().describe('New project ID'),
        project_name: z
          .string()
          .optional()
          .describe('New project name (partial, case-insensitive)'),
        service_id: z.number().int().positive().optional().describe('New service ID'),
        service_name: z
          .string()
          .optional()
